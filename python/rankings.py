...
for h in range(start_index, end_index + 1):
    ...
    rankings = []
    rankings.append(['week_title', 'week_year', 'week_month', 'week_day', 'rank_text', 'rank_number', 'move_positions', 'move_direction', 'player_age', 'ranking_points', 'tourneys_played', 'player_url', 'player_slug', 'name', 'country', 'player_id'])
    #for i in xrange(1160, 1170):
    for i in range(0, player_count):
        rank_text = rank_cleaned[i]
        rank_number = rank_text.replace('T', '')
        
        country_text = country_cleaned[i]
        
        name_text = player_name_cleaned[i]
